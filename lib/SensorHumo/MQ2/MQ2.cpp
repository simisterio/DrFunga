...
bool MQ2::checkCalibration() {
	if (Ro < 0.0) {
		DEBUG_MQ2_PRINTLN("Device not calibrated, call MQ2::begin before reading any value.");
		return false;
	}
...
float* MQ2::read(bool print){
	...
	if (print){
		DEBUG_MQ2_PRINT(lastReadTime);
		DEBUG_MQ2_PRINT("ms - LPG:");
		DEBUG_MQ2_PRINT(values[0]);
		DEBUG_MQ2_PRINT("ppm\t");
		DEBUG_MQ2_PRINT("CO:");
		DEBUG_MQ2_PRINT(values[1]);
		DEBUG_MQ2_PRINT("ppm\t");
		DEBUG_MQ2_PRINT("SMOKE:");
		DEBUG_MQ2_PRINT(values[2]);
		DEBUG_MQ2_PRINT("ppm\n");
	}
...
float MQ2::MQResistanceCalculation(int raw_adc) {
	...
	return RL_VALUE * (4095 - flt_adc) / flt_adc;
}
